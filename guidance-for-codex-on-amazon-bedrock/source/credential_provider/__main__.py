...
class MultiProviderAuth:
    def __init__(self, profile=None):
        ...
        self.profile = profile or self._auto_detect_profile() or "Codex"
    ...
    def _auto_detect_profile(self):
        ...
        try:
            ...
            if not config_path.exists():
                config_path = Path.home() / "codex-with-bedrock" / "config.json"
    ...
    def _load_config(self):
        ...
        2. ~/codex-with-bedrock/config.json (for installed packages)
        """
        # Try same directory as binary first (for testing)
        binary_dir = Path(__file__).parent if not getattr(sys, "frozen", False) else Path(sys.executable).parent
        config_path = binary_dir / "config.json"
        ...
        if not config_path.exists():
            raise ValueError(
                f"Configuration file not found in {binary_dir} or {Path.home() / 'codex-with-bedrock'}"
            )
        ...
        if profile_config.get("azure_auth_mode") == "secret":
            try:
                secret = keyring.get_password("codex-with-bedrock", f"{self.profile}-client-secret")
                if secret:
    ...
    def _init_credential_storage(self):
        ...
        if self.credential_storage == "session":
            ...
            self.cache_dir = Path.home() / "codex-with-bedrock" / "cache"
        ...
        if self.credential_storage == "keyring":
            try:
                ...
                if platform.system() == "Windows":
                    ...
                    keys_json = keyring.get_password("codex-with-bedrock", f"{self.profile}-keys")
                    token1 = keyring.get_password("codex-with-bedrock", f"{self.profile}-token1")
                    token2 = keyring.get_password("codex-with-bedrock", f"{self.profile}-token2")
                    meta_json = keyring.get_password("codex-with-bedrock", f"{self.profile}-meta")
                    ...
                    creds_json = keyring.get_password("codex-with-bedrock", f"{self.profile}-credentials")
        ...
        if self.credential_storage == "keyring":
            try:
                ...
                if platform.system() == "Windows":
                    ...
                    keyring.set_password("codex-with-bedrock", f"{self.profile}-token1", token[:mid])
                    keyring.set_password("codex-with-bedrock", f"{self.profile}-token2", token[mid:])
                    keyring.set_password(
                        "codex-with-bedrock",
                        f"{self.profile}-meta",
                    ...
                    keyring.set_password(
                        "codex-with-bedrock", f"{self.profile}-credentials", json.dumps(credentials)
                    )
            except Exception as e:
    ...
    def clear_cached_credentials(self):
        ...
        try:
            if platform.system() == "Windows":
                ...
                for entry in entries_to_clear:
                    if keyring.get_password("codex-with-bedrock", entry):
                        ...
                        keyring.set_password("codex-with-bedrock", entry, expired_data)

                cleared_items.append("keyring credentials (Windows)")
            else:
                # Non-Windows: single entry storage
                if keyring.get_password("codex-with-bedrock", f"{self.profile}-credentials"):
                    ...
                    keyring.set_password("codex-with-bedrock", f"{self.profile}-credentials", expired_credential)
        ...
        try:
            if keyring.get_password("codex-with-bedrock", f"{self.profile}-monitoring"):
                ...
                keyring.set_password("codex-with-bedrock", f"{self.profile}-monitoring", expired_token)
        ...
        session_dir = Path.home() / ".codex-session"
    ...
    def save_monitoring_token(self, id_token, token_claims):
        ...
        try:
            ...
            if self.credential_storage == "keyring":
                ...
                keyring.set_password("codex-with-bedrock", f"{self.profile}-monitoring", json.dumps(token_data))
            else:
            ...
            os.environ["CODEX_MONITORING_TOKEN"] = id_token
    ...
    def get_monitoring_token(self):
        ...
        try:
            ...
            env_token = os.environ.get("CODEX_MONITORING_TOKEN")
            if env_token:
                return env_token

            if self.credential_storage == "keyring":
                # Retrieve from keyring
                token_json = keyring.get_password("codex-with-bedrock", f"{self.profile}-monitoring")
            ...
            if exp_time - now > 60:
                ...
                os.environ["CODEX_MONITORING_TOKEN"] = token
                return token
    ...
    def save_to_credentials_file(self, credentials, profile="Codex"):
        """Save credentials to ~/.aws/credentials file

        Args:
            credentials: Dict with AccessKeyId, SecretAccessKey, SessionToken, Expiration
            profile: Profile name to use in credentials file (default: Codex)
        """
        import tempfile
    ...
    def read_from_credentials_file(self, profile="Codex"):
    ...
    def check_credentials_file_expiration(self, profile="Codex"):
    ...
    def authenticate_oidc(self):
        ...
        # NOTE: Signature verification disabled because token received directly from OIDC provider's
        # token endpoint over HTTPS after successful auth code exchange. The token is already
        # validated by AWS STS/Cognito when exchanged for credentials. This decode is only for
        # extracting claims (nonce, email, sub) for validation and logging.
    ...
    def get_aws_credentials_direct(self, id_token, token_claims):
        ...
        try:
            ...
            if "email" in token_claims:
                ...
                session_name = f"codex-{sub_sanitized}"
        ...
        while time.time() - start_time < timeout:
            ...
            try:
                ...
                if e.errno == errno.EADDRINUSE:
                    ...
                    time.sleep(0.5)  # nosemgrep: arbitrary-sleep - Polling delay for callback server
                else:
                    # Other error
                    raise
            finally:
    ...
    def _get_last_quota_check_time(self) -> datetime | None:
        ...
        try:
            if self.credential_storage == "keyring":
                timestamp_str = keyring.get_password("codex-with-bedrock", f"{self.profile}-quota-check")
                if timestamp_str:
    ...
    def _save_quota_check_timestamp(self):
        ...
        try:
            ...
            if self.credential_storage == "keyring":
                keyring.set_password("codex-with-bedrock", f"{self.profile}-quota-check", now)
            else:
                session_dir = Path.home() / ".codex-session"
                session_dir.mkdir(parents=True, exist_ok=True)
                timestamp_file = session_dir / f"{self.profile}-quota-check.json"
...
<html>
<head>
    <title>Quota Status - Codex</title>
    <style>
        body {{
...
<body>
    ...
    def _try_silent_refresh(self):
        ...
        try:
            ...
            # NOTE: Signature verification disabled for cached token that was already verified
            # when originally received from OIDC provider and validated by AWS STS/Cognito.
            # This decode is only for extracting claims for AWS credential exchange.
            # nosemgrep: unverified-jwt-decode
    ...
    def run(self):
        ...
        try:
            ...
            if "NotAuthorizedException" in error_msg and "Token is not from a supported provider" in error_msg:
                ...
                print("Please run 'poetry run cxwb init' to reconfigure.", file=sys.stderr)
...
def main():
    ...
    default_profile = os.getenv("CXWB_PROFILE", "Codex")
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "For non-interactive use set CXWB_CLIENT_SECRET env var before running; "
    ...
    # and process listings.  Use CXWB_CLIENT_SECRET env var for automation, or
    # the interactive getpass prompt for manual setup.
    if args.set_client_secret:
        import getpass

        env_secret = os.environ.get("CXWB_CLIENT_SECRET")
        if env_secret is not None:
            if not env_secret:
                print("Error: CXWB_CLIENT_SECRET is set but empty.", file=sys.stderr)
                sys.exit(1)
            secret = env_secret
        else:
        ...
        try:
            if not secret:
                try:
                    keyring.delete_password("codex-with-bedrock", f"{args.profile}-client-secret")
                ...
                keyring.set_password("codex-with-bedrock", f"{args.profile}-client-secret", secret)

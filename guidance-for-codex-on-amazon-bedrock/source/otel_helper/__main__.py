...
"""
OTEL Headers Helper Script for Codex
...
logger = logging.getLogger("codex-otel-headers")
...
def get_cache_path():
    ...
    cache_dir = Path.home() / ".codex-session"
    cache_dir.mkdir(parents=True, exist_ok=True, mode=0o700)
    profile = os.environ.get("AWS_PROFILE", "Codex")
...
def get_token_via_credential_process():
    ...
    if platform.system() == "Windows":
        credential_process = os.path.expanduser("~/codex-with-bedrock/credential-process.exe")
    else:
        credential_process = os.path.expanduser("~/codex-with-bedrock/credential-process")
    ...
    # Get profile name from AWS_PROFILE environment variable (set by Codex from settings.json)
    # Fall back to "Codex" for backward compatibility
    profile = os.environ.get("AWS_PROFILE", "Codex")
...
def main():
    ...
    token = os.environ.get("CODEX_MONITORING_TOKEN")
    if token:
        logger.info("Using token from environment variable CODEX_MONITORING_TOKEN")
    else:
    ...
    try:
        ...
        # Return failure on error - Codex should handle this gracefully
        return 1

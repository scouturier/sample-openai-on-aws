# ABOUTME: Configuration migration utilities for Codex with Bedrock
...
def migrate_legacy_config() -> bool:
    ...
    Migrates from:
        source/.cxwb-config/config.json (all profiles in one file)
    To:
        ~/.cxwb/config.json (global config with active profile)
        ~/.cxwb/profiles/*.json (individual profile files)
    ...
    try:
        ...
        active_profile_name = legacy_data.get("default_profile") or "Codex"

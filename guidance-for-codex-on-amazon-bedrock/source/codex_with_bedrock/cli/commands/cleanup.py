...
class CleanupCommand(Command):
    ...
    options = [
        ...
        option(
            "profile", description="AWS profile name to remove (default: Codex)", flag=False, default="Codex"
        ),
        option(
    ...
    def handle(self) -> int:
        ...
        auth_dir = Path.home() / "codex-with-bedrock"
        if auth_dir.exists():
        ...
        if codex_settings.exists():
            items_to_remove.append(("File", str(codex_settings), "Codex telemetry settings"))
        ...
        # Remove Codex settings if empty directory
        if codex_settings.exists():
            try:
                os.remove(codex_settings)
                console.print(f"✓ Removed {codex_settings}")

                # Remove .codex directory if empty
                codex_dir = codex_settings.parent
                if codex_dir.exists() and not any(codex_dir.iterdir()):
                    codex_dir.rmdir()
                    console.print(f"✓ Removed empty directory {codex_dir}")
            except Exception as e:
                console.print(f"[red]✗ Failed to remove Codex settings: {e}[/red]")

        console.print("\n[green]Cleanup completed![/green]")

        # Show next steps
        console.print("\n[bold]Next steps:[/bold]")
        console.print("• Run 'cxwb package' to create a new distribution")
        console.print("• Run 'cxwb test' to reinstall and test")
    ...
    def _clear_credentials_only(self, console, profile_name, force):
        ...
        credential_process = Path.home() / "codex-with-bedrock" / "credential-process"
        ...
        console.print("• Use 'export AWS_PROFILE=Codex' to set the profile")

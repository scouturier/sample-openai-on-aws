...
class DistributeCommand(Command):
    ...
    def handle(self) -> int:
        ...
        console.print(
            Panel.fit(
                "[bold cyan]Codex Package Distribution[/bold cyan]\n\nShare packages securely via presigned URLs",
                border_style="cyan",
                padding=(1, 2),
            )
        )
        ...
        if self._check_old_flat_structure(dist_dir):
            ...
            console.print("  [cyan]poetry run cxwb package --profile <profile-name>[/cyan]")
            console.print()
            return 1
        ...
        if not builds or all(len(b) == 0 for b in builds.values()):
            ...
            console.print("Run 'poetry run cxwb package' first to build packages.")
            return 1
        ...
        if profile.enable_distribution:
            ...
            try:
                ...
                if not dist_outputs:
                    ...
                    console.print("  poetry run cxwb deploy distribution")
                    return 1
            except Exception:
                console.print("[red]Distribution stack not deployed.[/red]")
                console.print("Deploy the distribution stack first:")
    ...
    def _get_latest_url(self, profile, console: Console) -> int:
        ...
        try:
            ...
            response = ssm.get_parameter(
                Name=f"/codex/{profile.identity_pool_name}/distribution/latest", WithDecryption=True
            )
            ...
            if expires < now:
                ...
                console.print("Generate a new one with: poetry run cxwb distribute")
                return 1
            ...
            filename = data.get("filename", "codex-package.zip")
            ...
            console.print(f"   unzip {filename} && cd codex-package && ./install.sh")
            ...
            console.print("   cd codex-package")
            ...
            if e.response["Error"]["Code"] == "ParameterNotFound":
                ...
                console.print("Create one with: poetry run cxwb distribute")
            else:
    ...
    def _upload_landing_page_packages(self, profile, console: Console, package_path: Path) -> int:
        ...
        try:
            ...
            console.print("Deploy the distribution stack first: poetry run cxwb deploy distribution")
            return 1
        ...
        if not available_platforms:
            ...
            console.print("Run: [cyan]poetry run cxwb package[/cyan] first")
            return 1
        ...
        # Path format: dist/3p-codex/2025-11-11-144312
        ...
        with Progress(
            ...
            for platform, files in available_platforms.items():
                ...
                with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zipf:
                    # Create codex-package directory in the ZIP
                    for source_file, archive_name in files:
                        source_path = package_path / source_file
                        if source_path.exists():
                            zipf.write(source_path, f"codex-package/{archive_name}")

                    # Include codex-settings if it exists
                    settings_dir = package_path / "codex-settings"
                    if settings_dir.exists() and settings_dir.is_dir():
                        for file in settings_dir.rglob("*"):
                            if file.is_file():
                                rel_path = file.relative_to(package_path)
                                zipf.write(file, f"codex-package/{rel_path}")
    ...
    def _create_distribution(self, profile, console: Console, package_path: Path) -> int:
        ...
        if not found_platforms:
            ...
            console.print("Run: [cyan]poetry run cxwb package --target-platform all[/cyan]")
            return 1
        ...
        with Progress(
            ...
            filename = f"codex-package-{timestamp}.zip"
            ...
            try:
                ssm.put_parameter(
                    Name=f"/codex/{profile.identity_pool_name}/distribution/latest",
                    Value=json.dumps(
                        {
                            "url": url,
                    ...
                    Type="SecureString",
                    Overwrite=True,
                    Description="Latest Codex package distribution URL",
                )
            except ClientError as e:
        ...
        if profile.enable_distribution:
            ...
            console.print("  3. Run: poetry run cxwb deploy distribution")
    ...
    def _create_archive(self, package_path: Path) -> Path:
        ...
        temp_dir = Path(tempfile.mkdtemp())
        archive_path = temp_dir / "codex-package.zip"

        # Create a clean package directory with only necessary files
        package_temp_dir = temp_dir / "codex-package"
        ...
        # Also include codex-settings directory if it exists
        settings_dir = package_path / "codex-settings"
        if settings_dir.exists() and settings_dir.is_dir():
            shutil.copytree(settings_dir, package_temp_dir / "codex-settings")
        ...
        # When extracted, it will create codex-package/ with files directly inside
        with zipfile.ZipFile(archive_path, "w", zipfile.ZIP_DEFLATED) as zf:
            ...
            for file in package_temp_dir.rglob("*"):
                if file.is_file():
                    ...
                    # This creates paths like "config.json", "install.sh" instead of "codex-package/config.json"
                    arcname = f"codex-package/{file.relative_to(package_temp_dir)}"

...
from codex_with_bedrock.cli.utils.display import display_configuration_info
...
class PackageCommand(Command):
    ...
    options = [
        ...
        option(
            "status",
            description="[DEPRECATED] Use 'cxwb builds' instead. Check build status by ID or 'latest'",
            flag=False,
            default=None,
        ),
    ...
    def handle(self) -> int:
        ...
        if self.option("status") is not None:
            ...
            console.print("  • [cyan]poetry run cxwb builds[/cyan]                    (list all recent builds)")
            console.print("  • [cyan]poetry run cxwb builds --status <build-id>[/cyan] (check specific build)")
            console.print("  • [cyan]poetry run cxwb builds --status latest[/cyan]    (check latest build)")
        ...
        config = Config.load()
        # Use specified profile or default to active profile, or fall back to "Codex"
        profile_name = self.option("profile") or config.active_profile or "Codex"
        profile = config.get_profile(profile_name)

        if not profile:
            console.print("[red]No deployment found. Run 'poetry run cxwb init' first.[/red]")
            return 1
        ...
        # Prompt for co-authorship preference (Codex defaults to true, allow opt-out)
        include_coauthored_by = questionary.confirm(
            "Include 'Co-Authored-By: Codex' in git commits?",
            default=True,
        ).ask()
        ...
        # Always create Codex settings (required for Bedrock configuration)
        console.print("[cyan]Creating Codex settings...[/cyan]")
        self._create_codex_settings(output_dir, profile, include_coauthored_by, profile_name)
        ...
        if profile.monitoring_enabled and (output_dir / "codex-settings" / "config.toml").exists():
            console.print("  • codex-settings/config.toml - Codex CLI configuration")
        ...
        if profile.enable_distribution:
            console.print("To create a distribution package: [cyan]poetry run cxwb distribute[/cyan]")
        else:
    ...
    def _check_build_status(self, build_id: str, console: Console) -> int:
        ...
        try:
            ...
            if not build_id or build_id == "latest":
                build_info_file = Path.home() / ".codex" / "latest-build.json"
                if not build_info_file.exists():
                    console.print("[red]No recent builds found. Start a build with 'poetry run cxwb package'[/red]")
                    return 1
            ...
            profile = config.get_profile(profile_name)
            if not profile:
                console.print("[red]No configuration found. Run 'poetry run cxwb init' first.[/red]")
                return 1
            ...
            if status == "IN_PROGRESS":
                ...
                console.print("  Run: [cyan]poetry run cxwb distribute[/cyan]")
    ...
    def _build_native_executable_nuitka(self, output_dir: Path, target_platform: str) -> Path:
        ...
        if target_platform == "macos":
            ...
            macos_variant = os.environ.get("CXWB_MACOS_VARIANT", "").lower()
        ...
        if target_platform == "macos":
            cmd.extend(
                [
                    ...
                    "--macos-app-name=Codex Credential Process",
        ...
        source_dir = Path(__file__).parent.parent.parent.parent
        # nosemgrep - subprocess uses safe list format
    ...
    def _build_macos_pyinstaller(self, output_dir: Path, arch: str) -> Path:
        ...
        if arch == "x86_64" and platform.machine().lower() == "arm64":
            ...
            if x86_venv_path.exists() and (x86_venv_path / "bin" / "pyinstaller").exists():
                ...
                console.print(
                    "[dim]https://github.com/aws-solutions-library-samples/guidance-for-codex-with-amazon-bedrock#optional-intel-mac-builds[/dim]\n"
                )
...
"""
            ...
            container_name = f"cxwb-extract-{arch}-{int(time.time())}"
...
"""
            ...
            image_tag = f"cxwb-otel-{arch}-builder-{int(time.time())}"

            # Remove any existing image with similar name to ensure fresh build
            if verbose:
                console.print("[dim]Cleaning up old Docker images...[/dim]")
            subprocess.run(
                ["docker", "rmi", "-f", f"cxwb-otel-{arch}-builder"],
                capture_output=True,
            )
            ...
            container_name = f"cxwb-otel-extract-{arch}-{int(time.time())}"
    ...
    def _build_windows_via_codebuild(self, output_dir: Path) -> Path:
                ...
                if response.get("ids"):
                    ...
                    for build in builds_response.get("builds", []):
                        if build["buildStatus"] == "IN_PROGRESS":
                            ...
                            console.print("Check status: [cyan]poetry run cxwb builds[/cyan]")
        ...
        if not profile or not profile.enable_codebuild:
            ...
            console.print("  1. Run: poetry run cxwb init")
            console.print("  2. Answer 'Yes' when asked about Windows build support")
            console.print("  3. Run: poetry run cxwb deploy codebuild")
        ...
        try:
            ...
            console.print("Run: poetry run cxwb deploy codebuild")
        ...
        console.print("  [cyan]poetry run cxwb builds[/cyan]")
        ...
        console.print("  2. Run [cyan]poetry run cxwb builds[/cyan] to check completion status")
        console.print("  3. Once complete, run [cyan]poetry run cxwb distribute[/cyan]")
    ...
    def _build_native_otel_helper(self, output_dir: Path, target_platform: str) -> Path:
        ...
        if target_platform == "macos":
            cmd.extend(
                [
                    "--macos-create-app-bundle",
                    "--macos-app-name=Codex OTEL Helper",
                    "--disable-console",
                ]
            )
        elif target_platform == "linux":
    ...
    def _create_config(
        ...
        Args:
            ...
            profile_name: Name to use as key in config.json (defaults to "Codex" for backward compatibility)
        """
        config = {
            profile_name: {
                "provider_domain": profile.provider_domain,
        ...
        if getattr(profile, "client_certificate_path", None):
            ...
            if (cert_is_absolute or key_is_absolute) and console:
                ...
                console.print("[dim]  AZURE_CLIENT_CERTIFICATE_KEY_PATH=<path/to/cert.pem>[/dim]\n")
...
# Codex Authentication Installer
...
echo "======================================"
echo "Codex Authentication Installer"
echo "======================================"
echo
...
mkdir -p ~/codex-with-bedrock

# Copy appropriate binary
cp "$CREDENTIAL_BINARY" ~/codex-with-bedrock/credential-process

# Copy config
cp config.json ~/codex-with-bedrock/
chmod +x ~/codex-with-bedrock/credential-process
...
if [[ "$OSTYPE" == "darwin"* ]]; then
    ...
    xattr -d com.apple.quarantine ~/codex-with-bedrock/credential-process 2>/dev/null || true
    echo
...
# Copy Codex settings if present
if [ -d "codex-settings" ]; then
    echo
    echo "Installing Codex settings..."
    mkdir -p ~/.codex

    # Copy config.toml and replace placeholders
    if [ -f "codex-settings/config.toml" ]; then
        # Check if config file already exists
        if [ -f ~/.codex/config.toml ]; then
            echo "Existing Codex config found"
            ...
            if [[ ! $REPLY =~ ^[Yy]$ ]]; then
                echo "Skipping Codex config..."
                SKIP_SETTINGS=true
            fi
        fi

        if [ "$SKIP_SETTINGS" != "true" ]; then
            # Replace placeholders and write config
            sed -e "s|__OTEL_HELPER_PATH__|$HOME/codex-with-bedrock/otel-helper|g" \
                "codex-settings/config.toml" > ~/.codex/config.toml
...
if [ -f "$OTEL_BINARY" ]; then
    ...
    cp "$OTEL_BINARY" ~/codex-with-bedrock/otel-helper-bin
    chmod +x ~/codex-with-bedrock/otel-helper-bin
    xattr -d com.apple.quarantine ~/codex-with-bedrock/otel-helper-bin 2>/dev/null || true
    # Install shell wrapper as otel-helper (fast cache check, avoids PyInstaller startup)
    if [ -f "otel-helper.sh" ]; then
        cp "otel-helper.sh" ~/codex-with-bedrock/otel-helper
        chmod +x ~/codex-with-bedrock/otel-helper
    else
        # Fallback: if shell wrapper not in package, point directly to binary
        cp "$OTEL_BINARY" ~/codex-with-bedrock/otel-helper
        chmod +x ~/codex-with-bedrock/otel-helper
    fi
...
if [ -f ~/codex-with-bedrock/otel-helper ]; then
    ...
    echo "  ~/codex-with-bedrock/otel-helper-bin --test"
fi
...
credential_process = $HOME/codex-with-bedrock/credential-process --profile $PROFILE_NAME
...
echo
echo "To use Codex authentication:"
...
REM Codex Authentication Installer for Windows
...
echo Installing authentication tools...
if not exist "%USERPROFILE%\\codex-with-bedrock" mkdir "%USERPROFILE%\\codex-with-bedrock"

REM Copy credential process executable with renamed target
echo Copying credential process...
copy /Y "credential-process-windows.exe" "%USERPROFILE%\\codex-with-bedrock\\credential-process.exe" >nul
if %errorlevel% neq 0 (
...
if exist "otel-helper-windows.exe" (
    ...
    copy /Y "otel-helper-windows.exe" "%USERPROFILE%\\codex-with-bedrock\\otel-helper.exe" >nul
)

REM Copy configuration
echo Copying configuration...
copy /Y "config.json" "%USERPROFILE%\\codex-with-bedrock\\" >nul

REM Copy Codex config if present
if exist "codex-settings" (
    echo Copying Codex config...
    if not exist "%USERPROFILE%\\.codex" mkdir "%USERPROFILE%\\.codex"

    REM Copy config.toml and replace placeholders
    if exist "codex-settings\\config.toml" (
        set SKIP_SETTINGS=false
        if exist "%USERPROFILE%\\.codex\\config.toml" (
            echo Existing Codex config found
            set /p OVERWRITE="Overwrite with new config? (y/n): "
        ...
        if not "%SKIP_SETTINGS%"=="true" (
            ...
            powershell -Command ^
            "$otelPath = '%USERPROFILE%\\\\codex-with-bedrock\\\\otel-helper.exe' ^
            -replace '\\\\\\\\', '/'; ^
            (Get-Content 'codex-settings\\\\config.toml') ^
            -replace '__OTEL_HELPER_PATH__', $otelPath | ^
            Set-Content '%USERPROFILE%\\\\.codex\\\\config.toml'"
            echo OK Codex config installed
        )
    )
)
...
"& {{$c=Get-Content config.json|ConvertFrom-Json;$c.PSObject.Properties.Name}}"') do (
    ...
    "%USERPROFILE%\\codex-with-bedrock\\credential-process.exe --profile %%p" --profile %%p
...
"""
    ...
    def _create_documentation(self, output_dir: Path, profile, timestamp: str):
        ...
        readme_content = f"""# Codex Authentication Setup
...
Invoke-WebRequest -Uri "URL_PROVIDED" -OutFile "codex-package.zip"
```

#### Step 2: Extract the Package

**Option A: Using Windows Explorer**
1. Right-click on `codex-package.zip`
...
Expand-Archive -Path "codex-package.zip" -DestinationPath "codex-package"
...
tar -xf codex-package.zip

# Or use PowerShell from Command Prompt
powershell -command "Expand-Archive -Path 'codex-package.zip' -DestinationPath 'codex-package'"
...
- Copy authentication tools to `%USERPROFILE%\\codex-with-bedrock`
- Configure the AWS profile "Codex"
- Test the authentication

#### Step 4: Use Codex
```cmd
# Set the AWS profile
set AWS_PROFILE=Codex
...
```powershell
$env:AWS_PROFILE = "Codex"
aws sts get-caller-identity
```

## What This Does

- Installs the Codex authentication tools
...
- Ensure you're assigned to the Codex application in your identity provider
...
Your organization has enabled advanced analytics for Codex usage. You can access detailed metrics \
...
"""
    ...
    def _create_codex_settings(
        self, output_dir: Path, profile, include_coauthored_by: bool = True, profile_name: str = "Codex"
    ):
        """Create Codex config.toml for the amazon-bedrock provider."""
        console = Console()

        try:
            # Create codex-settings directory (visible, not hidden)
            ...
            # Codex CLI uses ~/.codex/config.toml with the amazon-bedrock provider.
            # Authentication is handled via the AWS profile set up by the installer.
            toml_lines = [
                f'model = "{model_id}"',
                ...
                f'profile = "Codex"',
                f'region = "{profile.aws_region}"',
            ]

            if profile.monitoring_enabled:
                # Get monitoring stack outputs for OTEL endpoint
                ...
                if result.returncode == 0:
                    ...
                    if endpoint:
                        toml_lines += [
                            ...
                            'otel_headers_helper = "__OTEL_HELPER_PATH__"',
                        ]
            ...
            console.print("[dim]Created Codex config.toml for amazon-bedrock provider[/dim]")

...
class TestCommand(Command):
    ...
    def handle(self) -> int:
        ...
        if not test_profile_name:
            ...
            console.print(
                "Use [cyan]cxwb context use <profile>[/cyan] to set an active profile or use "
        ...
        console.print(
            Panel.fit(
                "[bold cyan]Codex Package Test[/bold cyan]\n\n"
        ...
        if not package_dir:
            console.print("[red]No package found. Run 'poetry run cxwb package' first.[/red]")
        ...
        with open(config_path) as f:
            ...
            # Try to read from the specified profile name, fall back to "Codex" for backward compatibility
            profile_config = pkg_config.get(test_profile_name) or pkg_config.get("Codex", {})
        ...
        if quota_only:
            # Load the profile from cxwb config
            ...
            test_profile = f"cxwb-test-{uuid.uuid4().hex[:8]}"
            credential_command = f"/bin/sh -c 'CXWB_PROFILE={test_profile_name} {credential_binary}'"
        ...
        if not profile:
            console.print(
                f"[red]Profile '{test_profile_name}' not found in configuration. Run 'poetry run cxwb \
                init' first.[/red]"
            )
            return 1

        # Use test_profile instead of hardcoded "Codex"
        ...
        with Progress(
            ...
            if result["status"] == "✓":
                ...
                if not profile.selected_source_region:
                    test_results.append(
                        ("Configuration", "✗", "selected_source_region not set - run 'cxwb init' to configure")
                    )
                    progress.stop()
                ...
                if quota_enabled and quota_endpoint:
                    ...
                    # Get profile name from package's config.json (more reliable than cxwb profile name)
        ...
        if failed > 0:
            ...
            if bedrock_failed:
                ...
                console.print("2. Check if you have access to Bedrock models")
                ...
                console.print(
                    ...
                    f"\"modelSummaries[?contains(modelId, 'gpt-oss')]\""
                )
    ...
    def _test_bedrock_access(self, profile_name: str, region: str, with_api: bool = False, selected_model: str = None) -> dict:
        ...
        try:
            ...
            describe_cmd = [
                ...
                region,
                "--query",
                "modelSummaries[?contains(modelId, 'gpt-oss')].modelId",
                "--output",
                "json",
            ]
            # nosemgrep - subprocess uses safe list format
                    ...
                    if with_api:
                        ...
                        if test_result["success"]:
                            ...
                            if "ValidationException" in error:
                                ...
                                return {
                                    "status": "✓",
                                    "details": f"Found {len(models)} Bedrock models (some models may \
                                    not support invoke)",
                                }
                                ...
                                return {
                                    "status": "✓",
                                    "details": f"Found {len(models)} Bedrock models (API test rate limited)",
                                }
                            elif "timeout" in error.lower():
                                # Timeouts could be transient
                                return {
                                    "status": "!",
                                    "details": f"Found {len(models)} Bedrock models (API test timed out)",
                                }
                            else:
                                # Other errors are actual failures
                                return {"status": "✗", "details": f"Found models but API test failed: {error[:80]}"}
                    else:
                        return {"status": "✓", "details": f"Found {len(models)} Bedrock models"}
                else:
                    return {"status": "!", "details": "No Bedrock models found"}
            else:
    ...
    def _test_otel_helper(self, otel_binary: Path, credential_binary: Path) -> dict:
        ...
        try:
            ...
            env = os.environ.copy()
            env["CODEX_MONITORING_TOKEN"] = token_result.stdout.strip()
    ...
    def _test_model_invocation(self, profile_name: str, region: str, selected_model: str = None) -> dict:
        ...
        try:
            ...
            if not selected_model:
                return {"success": False, "error": "No model configured - run 'cxwb init' to select a model"}

            model_id = selected_model

            # Create a minimal test prompt using OpenAI Messages API format
            body_dict = {
            ...
            if result.returncode == 0:
                # Check if we got a response (OpenAI format)
                try:
    ...
    def _test_quota_policies(self, profile) -> list:
        ...
        results = []
        test_email = f"cxwb-test-{uuid.uuid4().hex[:8]}@test.local"

        try:
            from codex_with_bedrock.models import EnforcementMode, PolicyType
            from codex_with_bedrock.quota_policies import QuotaPolicyManager
    ...
    def _invoke_metrics_aggregator(self, profile) -> dict:
        ...
        try:
            ...
            function_name = "Codex-MetricsAggregator"
            ...
            try:
                ...
                result = subprocess.run(
                    [
                        ...
                        "invoke-model",
                        "--model-id",
                        selected_model or "openai.gpt-5.4",
                        "--body",
                        f"fileb://{body_file}",
                        "--content-type",
                        "application/json",
                ...
                if result.returncode == 0:
                    model_short = (selected_model or "bedrock-llm").split(".")[-1][:30]
    ...
    def _test_usage_capture(
        ...
        try:
            ...
            # CloudWatch Logs has eventual consistency - metrics may take 1-2 seconds to appear
            time.sleep(2)  # nosemgrep: arbitrary-sleep - Required for CloudWatch Logs sync
            ...
            # Lambda + CloudWatch Logs query can take 3-5 seconds for full propagation
            time.sleep(5)  # nosemgrep: arbitrary-sleep - Required for Lambda execution + logs query

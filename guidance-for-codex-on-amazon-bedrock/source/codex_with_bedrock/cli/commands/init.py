...
# ABOUTME: Guides through complete Codex with Bedrock deployment
...
from codex_with_bedrock.cli.utils.aws import (
...
from codex_with_bedrock.cli.utils.progress import WizardProgress
from codex_with_bedrock.cli.utils.validators import (
...
class InitCommand(Command):
    ...
    def handle(self) -> int:
        ...
        try:
            ...
            console.print("Run [bold cyan]poetry run cxwb init[/bold cyan] to resume where you left off.")
            return 1
    ...
    def _handle_with_progress(self, console: Console, progress: WizardProgress) -> int:
        ...
        if existing_config and user_action == "update":
            ...
            console.print("\nNext steps:")
            console.print("• Deploy infrastructure: [cyan]poetry run cxwb deploy[/cyan]")
            console.print("• Create package: [cyan]poetry run cxwb package[/cyan]")
            console.print("• Test authentication: [cyan]poetry run cxwb test[/cyan]")
            return 0
        ...
        # Welcome message
        welcome = Panel.fit(
            "[bold cyan]Welcome to Codex with Bedrock Setup![/bold cyan]\n\n"
            "This wizard will help you deploy Codex using Amazon Bedrock with:\n"
        ...
        success_panel = Panel.fit(
            ...
            "Next steps:\n"
            "1. Deploy infrastructure: [cyan]poetry run cxwb deploy[/cyan]\n"
            "2. Create package: [cyan]poetry run cxwb package[/cyan]\n"
            "3. Test authentication: [cyan]poetry run cxwb test[/cyan]\n"
            f"4. View profile: [cyan]poetry run cxwb context show {profile_name}[/cyan]",
            border_style="green",
            padding=(1, 2),
        )
        console.print("\n", success_panel)
    ...
    def _gather_configuration(self, progress: WizardProgress, existing_config: dict[str, Any] = None, profile_name: str | None = None) -> dict[str, Any]:
        ...
        if not skip_okta:
            ...
            if provider_type == "azure":
                ...
                if auth_mode == "secret":
                    ...
                    _keyring.set_password("codex-with-bedrock", f"{profile_name}-client-secret", client_secret)
        ...
        if not skip_aws:
            ...
            if federation_type == "direct":
                stack_base_name = questionary.text(
                    ...
                    default=config.get("aws", {}).get("identity_pool_name", "codex-auth"),
        ...
        if not skip_monitoring:
            ...
            console.print("Track Codex usage and performance metrics in CloudWatch")
        ...
        console.print("Choose how to distribute Codex packages to end users:")
        ...
        if not skip_bedrock:
            ...
            if saved_model:
                ...
                for key, model_info in BEDROCK_MODELS.items():
            ...
            # Step 1: Select Bedrock model
            model_choices = []
            default_model_key = saved_model_key or "gpt-5.4"
            for model_key, model_info in BEDROCK_MODELS.items():
            ...
            selected_model = BEDROCK_MODELS[selected_model_key]
    ...
    def _review_configuration(self, config: dict[str, Any]) -> bool:
        ...
        if selected_model:
            table.add_row("Bedrock Model", model_display.get(selected_model, selected_model))

        # Show Bedrock Mantle regions
        table.add_row("Bedrock Regions", "US — us-east-1, us-east-2, us-west-2")
    ...
    def _deploy(self, config: dict[str, Any], profile_name: str = "default") -> int:
        ...
        success_panel = Panel.fit(
            ...
            "Next steps:\n"
            "1. Create package: [cyan]poetry run cxwb package[/cyan]\n"
            "2. Test authentication: [cyan]poetry run cxwb test[/cyan]\n"
    ...
    def _show_existing_deployment(self, config: dict[str, Any]) -> None:
        ...
        if selected_model:
            ...
            console.print(f"• Bedrock Model: [cyan]{model_names.get(selected_model, selected_model)}[/cyan]")

        console.print("• Bedrock Regions: [cyan]US — us-east-1, us-east-2, us-west-2[/cyan]")
    ...
    def _configure_vpc(self, region: str, existing_vpc_config: dict[str, Any] = None) -> dict[str, Any]:
        ...
        try:
            ...
            if profile and profile.stack_names:
                ...
                if monitoring_stack:
                    from codex_with_bedrock.cli.utils.aws import check_stack_exists, get_stack_outputs
    ...
    def _select_or_create_profile(self, console: Console) -> tuple[str, bool, str]:
        ...
        if action == choices[0]:  # Create new
            ...
            console.print("\nNext steps:")
            console.print("• Deploy infrastructure: [cyan]poetry run cxwb deploy[/cyan]")
            console.print("• View profile details: [cyan]poetry run cxwb context show[/cyan]")

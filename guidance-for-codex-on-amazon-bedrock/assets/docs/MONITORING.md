# Codex Monitoring Implementation

This guide explains how to deploy and use the optional monitoring system for tracking Codex usage through Amazon Bedrock.

When you enable monitoring during deployment, the system creates infrastructure to collect and visualize usage metrics from Codex. The monitoring stack deploys an OpenTelemetry (OTEL) Collector on AWS ECS Fargate that receives metrics from Codex and forwards them to CloudWatch for visualization and analysis.

## Architecture

The monitoring system consists of several components working together. Codex sends metrics using the OpenTelemetry Protocol (OTLP) to an Application Load Balancer. The ALB forwards these metrics to an OTEL Collector running on ECS Fargate. The collector then converts the metrics to CloudWatch's Embedded Metric Format (EMF) and sends them to CloudWatch Metrics and Logs. Finally, a CloudWatch Dashboard visualizes these metrics.
...
The CloudWatch Dashboard provides comprehensive visualization of your Codex usage. The dashboard uses Lambda functions and DynamoDB for efficient metrics collection and display, presenting real-time and historical usage data through custom widgets.
...
Codex sends several metric types that the collector processes:

- `codex.token.usage` - Tracks input and output token consumption
- `codex.session.count` - Counts active sessions
- `codex.active_time.total` - Measures time spent actively using Codex
- `codex.cost.usage` - Estimates costs based on token usage
- `codex.code_edit_tool.decision` - Records code editing decisions
...
Monitoring deployment happens during the initial setup when you run `poetry run cxwb init`. The interactive wizard prompts you to enable monitoring and configure the necessary infrastructure.
...
If you provide a custom domain name and hosted zone ID during setup, the system automatically provisions an ACM certificate and configures HTTPS. This ensures encrypted transmission of metrics from Codex to your collector.
...
The package command generates a `codex-settings/settings.json` file in the distribution package that configures Codex for telemetry collection. During installation, this file gets copied to `~/.codex/settings.json` in the user's home directory and contains all the settings needed for monitoring.
...
```json
{
  ...
  "otelHeadersHelper": "~/codex-with-bedrock/otel-helper"
}
```
...
The monitoring system tracks comprehensive metrics about Codex usage, with each metric tagged with user and organizational attributes for detailed analysis.

For token usage, the system tracks `codex.tokens.input` for input tokens per request, `codex.tokens.output` for generated output tokens, and `codex.tokens.total` for combined token usage. These metrics help you understand consumption patterns and costs.

Request metrics include `codex.requests.count` to track the number of API calls, `codex.requests.duration` to measure response times in milliseconds, and `codex.requests.errors` to monitor failed requests. These help identify performance issues and error patterns.

Each metric includes multiple dimensions for filtering and grouping. The `UserEmail` dimension comes from the OIDC token, allowing you to track individual user consumption. The `Model` dimension shows which Bedrock model was used (like claude-3-sonnet or claude-3-opus). The `Region` dimension indicates the AWS region where Bedrock was accessed.
...
The CloudWatch Dashboard named `CodexMonitoring` provides comprehensive visualization of your Codex metrics.

![Codex Monitoring Dashboard](/assets/images/CodexDashboard.png)
...
During installation, the `install.sh` script copies all necessary files including the Codex settings and OTEL helper binary. Users don't need to configure anything - the monitoring settings are pre-configured with your organization's collector endpoint.

When users work with Codex, metrics are sent in the background without affecting performance or user experience. The OTEL helper binary automatically extracts user information from their authentication token and includes it with metrics for attribution.
...
The Application Load Balancer is internet-facing to receive metrics from Codex installations, while ECS tasks run in private subnets for enhanced security. Security groups restrict access to only necessary ports and protocols, following the principle of least privilege.

## Summary

The monitoring system provides comprehensive visibility into Codex usage across your organization. Deployment is automated through the `cxwb` CLI tools, creating all necessary infrastructure with minimal configuration. The OTEL Collector on ECS Fargate handles metric collection and transformation, while CloudWatch provides storage and visualization.

...
1. Admin runs `poetry run cxwb distribute`
...
1. Run `poetry run cxwb init`
2. Select "Presigned S3 URLs (simple, no authentication)"
3. Run `poetry run cxwb deploy distribution`
...
1. Run `poetry run cxwb init` (reconfigure)
...
A: Run `cxwb init` to reconfigure, then `cxwb deploy distribution` to update the stack.

**Q: Do both types work with the same `cxwb distribute` command?**
...
A: For presigned-s3: Generate new URLs with `cxwb distribute`. For landing-page: URLs regenerate automatically when users visit.

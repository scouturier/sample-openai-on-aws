...
Complete guide for deploying Codex package distribution using either presigned S3 URLs (simple) or authenticated landing pages (enterprise).

---

## Overview

Codex with Bedrock supports **two distribution methods** for sharing packaged binaries and settings with end users:
...
- **Python 3.10+**: Required for cxwb CLI
- **Poetry**: Python package manager (`curl -sSL https://install.python-poetry.org | python3 -`)
- **Basic Authentication Configured**: Must have completed `cxwb init` for Bedrock authentication
- **Packages Built**: Run `cxwb package` to create distribution packages
...
- **VPC with Subnets**:
  ...
  - Can be created via `cxwb deploy networking` or use existing VPC
...
- Save configuration to `~/.cxwb/profiles/<profile-name>.json`
...
**URL expires after 7 days** - regenerate by running `cxwb distribute` again.
...
2. **Phase 2 (DURING)**: Deploy infrastructure via cxwb
...
### Phase 1: IdP Web Application Setup (BEFORE cxwb init)
...
| **Created When** | During first cxwb init           | Before distribution setup              |
...
3. **Configure Application Settings**:
   ...
   - **App integration name**: `Codex Distribution`
...
4. **Create Client Secret**:
   ...
   - Description: `Codex Distribution Landing Page`
...
When you run `poetry run cxwb init` (Phase 2), the wizard will:
...
**Option B: Create new VPC via cxwb**:

```bash
poetry run cxwb deploy networking
```
...
2. Select **"Codex Distribution"** application
...
**No need to regenerate URLs** - the landing page URL is permanent. Only update packages by running `cxwb distribute` when you have new versions.
...
```bash
# macOS only
poetry run cxwb package --target-platform macos

# Windows only (requires CodeBuild)
poetry run cxwb package --target-platform windows

# Linux only
poetry run cxwb package --target-platform linux
```
...
- Codex settings directory (if configured)
...
```bash
# 48 hours (default)
poetry run cxwb distribute --expires-hours 48

# 1 hour (minimum)
poetry run cxwb distribute --expires-hours 1

# 7 days (168 hours, maximum)
poetry run cxwb distribute --expires-hours 168
```
...
**Version**: 1.0.0
**Compatibility**: Codex with Bedrock v1.0+

...
During `cxwb init`, quota monitoring is **enabled by default** when monitoring is enabled. You'll be prompted to configure:
...
Deploy using `poetry run cxwb deploy` (deploys all enabled stacks) or `poetry run cxwb deploy quota` for just the quota stack. The OIDC configuration is automatically passed from your profile settings. For complete deployment instructions, see the [CLI Reference](CLI_REFERENCE.md#deploy---deploy-infrastructure).
...
To update limits: Re-run `cxwb init` and redeploy with `cxwb deploy quota`.
...
Use the `cxwb quota` commands to manage policies:

```bash
# Set a user-specific policy
cxwb quota set-user john.doe@company.com --monthly-limit 500M --daily-limit 20M

# Set a group policy
cxwb quota set-group engineering --monthly-limit 400M

# Set the default policy for all users
cxwb quota set-default --monthly-limit 225M --daily-limit 8M

# List all policies
cxwb quota list
cxwb quota list --type group

# Show effective policy for a user
cxwb quota show john.doe@company.com --groups "engineering,ml-team"

# View current usage against limits
cxwb quota usage john.doe@company.com

# Delete a policy
cxwb quota delete group engineering

# Temporarily unblock a user who exceeded quota (Phase 2)
cxwb quota unblock john.doe@company.com --duration 24h
```
...
cxwb quota export policies.json

# Export to CSV for spreadsheet editing
cxwb quota export policies.csv

# Export only user policies
cxwb quota export users.json --type user
```
...
cxwb quota import users.csv --update

# Preview changes without applying
cxwb quota import users.csv --dry-run

# Auto-calculate daily limits (monthly / 30 + burst buffer)
cxwb quota import users.csv --auto-daily --burst 15
```
...
1. **Initial setup from HR system:**
   ...
   cxwb quota import users.csv --auto-daily --update
   ```

2. **Backup before changes:**
   ```bash
   cxwb quota export backup-$(date +%Y%m%d).json
   ```

3. **Cross-environment sync:**
   ```bash
   # Export from staging
   cxwb quota export policies.json --profile staging

   # Import to production
   cxwb quota import policies.json --profile production --update
   ```
...
```bash
# View Lambda logs
aws logs tail /aws/lambda/codex-quota-monitor --follow
...
4. Optionally create a default policy to maintain previous behavior:
   ```bash
   cxwb quota set-default --monthly-limit 225M
   ```
...
cxwb quota set-user john.doe@company.com --monthly-limit 10M --enforcement block

# Set group policy with blocking
cxwb quota set-group engineering --monthly-limit 50M --enforcement block

# Set default with blocking
cxwb quota set-default --monthly-limit 225M --enforcement block
```
...
cxwb quota unblock john.doe@company.com

# Unblock for 7 days
cxwb quota unblock john.doe@company.com --duration 7d

# Unblock until end of month (quota reset)
cxwb quota unblock john.doe@company.com --duration until-reset

# With reason
cxwb quota unblock john.doe@company.com --duration 24h --reason "Urgent project deadline"
```
...
When using `cxwb deploy quota`, the OIDC configuration is **automatically passed** from your profile settings (configured during `cxwb init`). No manual parameter configuration is required.

For manual CloudFormation deployments, provide your OIDC configuration:

```bash
aws cloudformation deploy \
  --stack-name codex-quota \
...
If a user exceeds their quota mid-session, they can continue using Codex until their credentials expire and they need to re-authenticate. At that point, the quota check will block access.

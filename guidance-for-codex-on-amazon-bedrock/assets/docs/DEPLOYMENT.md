...
This guide walks IT administrators through deploying Codex authentication across your organization, transforming your existing identity provider into a gateway for secure Amazon Bedrock access.
...
Deploying Codex authentication involves four key phases: configuring your identity provider, deploying AWS infrastructure, creating distribution packages, and supporting your users. Each phase builds on the previous one, creating a complete authentication solution that's transparent to end users.

## Phase 1: Configuring Your Identity Provider

The journey begins in your organization's identity provider console. Whether you're using Okta, Azure AD, or Auth0, you'll create a new application that serves as the authentication gateway for Codex.

Log into your provider's admin console and navigate to the application creation section. You're creating what's known as a "Native Application" in OIDC terms - this tells the provider that users will authenticate from their local machines rather than a web server. Name it something clear like "Codex Authentication" or "Amazon Bedrock CLI Access" so users recognize it during login.
...
Next, determine who should have access. The cleanest approach is creating a dedicated group like "Codex Users" and assigning it to the application. This gives you centralized control over access - simply add users to the group to grant access, or remove them to revoke it. Apply any additional policies your organization requires, such as MFA or device trust requirements.
...
The `cxwb` (Codex with Bedrock) CLI tool guides you through deployment with an interactive wizard. Run `poetry run cxwb init` to begin. The wizard walks you through each configuration decision, starting with your OIDC provider details - enter the domain and Client ID you noted earlier.
...
Codex supports building for all major platforms:
...
poetry run cxwb package --target-platform=windows    # Windows via CodeBuild
poetry run cxwb package --target-platform=macos      # Current macOS architecture
poetry run cxwb package --target-platform=linux      # Linux via Docker
```
...
- If you enabled monitoring, OTEL helper executables and Codex telemetry settings that point to your OpenTelemetry collector
...
1. **Enable during init**: When running `poetry run cxwb init`, you'll be prompted:
...
2. **If enabled**, Windows builds will automatically trigger when you run:
   ...
   poetry run cxwb package --target-platform=windows
   ```
...
- To enable Windows builds after initial setup, re-run `poetry run cxwb init`
...
```bash
poetry run cxwb test --api
```

## Phase 5: Distributing to Your Users

With a tested package in hand, you're ready for the final phase: getting the authentication system to your users. Codex offers two distribution methods:
...
When they run Codex with `AWS_PROFILE=Codex`, authentication happens automatically in the background. On first use, users will see a browser window open for authentication with your organization's identity provider.
